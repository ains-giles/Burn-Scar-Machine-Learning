...
'''
x_path = '/Users/ainsleyg/Documents/CISESS/burn_composites/combined_rectangles_08_10_21.h5'
y_path = '/Users/ainsleyg/Documents/CISESS/burn_composites/boolean_burn_composite_training.h5'
...
#     max_length = max(img.shape[0] for img in X)
#     max_width = max(img.shape[1] for img in X)
    
#     max_length = (max_length + 15) // 16 * 16
#     max_width = (max_width + 15) // 16 * 16
    
#     X_padded = [np.pad(img, ((0, max_length - img.shape[0]), (0, max_width - img.shape[1]), (0, 0)), mode='constant', constant_values=0) for img in X]
#     y_padded = [np.pad(mask, ((0, max_length - mask.shape[0]), (0, max_width - mask.shape[1]), (0, 0)), mode='constant', constant_values=0) for mask in y]

#     return X_padded, y_padded, max_length, max_width
...
def load_data_with_padding(x_path, y_path):
    ...
    def process_image(image, target_height, target_width):
        ...
        if height > target_height:
            ...
            image = np.pad(image, ((pad_y, target_height - height - pad_y), (0, 0), (0, 0)), mode='constant', constant_values=0)
        ...
        if width > target_width:
            ...
            image = np.pad(image, ((0, 0), (pad_x, target_width - width - pad_x), (0, 0)), mode='constant', constant_values=0)
        
        return image
    
    X_processed = [process_image(img, target_height, target_width) for img in X]
    y_processed = [process_image(mask, target_height, target_width) for mask in y]

    return np.array(X_processed), np.array(y_processed)

X, y = load_data_with_padding(x_path, y_path)
print("X shape:", X.shape)  # Should print (27, 160, 128, 3)
print("y shape:", y.shape)  # Should print (27, 160, 128, 1)


#print(len(X))
#print(len(y))
np.save('/Users/ainsleyg/Documents/CISESS/X_desert_08_10_21.npy', X)
np.save('/Users/ainsleyg/Documents/CISESS/y_desert_08_10_21.npy', y)
...
'''
unet_cnn_model.fit(X_train, y_train, validation_data=(X_val,y_val), batch_size=16, epochs=100, callbacks=callbacks)
...
unet_cnn_model.save('/Users/ainsleyg/Documents/CISESS/unet_model_08_10_21.h5')
